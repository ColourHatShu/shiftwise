...
- [ ] Make backend integration suites (worker-*, security-pipeline) runnable without a live Postgres — they're the remaining 27 failures. Options: a CI Postgres service, a dedicated Supabase **test** project, or mocking prisma. Likely a test-infra decision (don't point them at the real Supabase DB) — pairs well with the GitHub Actions CI item.
