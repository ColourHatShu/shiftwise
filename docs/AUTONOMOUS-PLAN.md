...
- [ ] **Finish modal consolidation — needs a design decision (human/council):** remaining modals split into two visual languages — generic Tailwind (`ShiftModal`, `BulkUploadModal`, `AuditPackModal`: `rounded-lg`/`shadow-lg`/`max-h-96`) vs design-system (`EditWorkerModal`, `WorkerDetailModal`: `rounded-xl`/`shadow-2xl`/`backdrop-blur`). Pick the canonical modal style + standardize max-h/shadow/rounded, then migrate them onto `<Modal>`. Migrating as-is would cause visible regressions.
